...
#from web3 import Web3 allows us to simulate interactions with an Ethereum blockchain
#“mock” blockchain that the ethereum-tester library and the EthereumTesterProvider 
#makes available to developers for testing purposes.
...
#To communicate the transaction to the mock development blockchain, 
#we’ll use the eth.sendTransaction function, and we’ll specify the values for the parameters to, from, and value. 
...
#get_people(w3)provide us with Python functions for performing various blockchain operations, 
#like accessing information on the latest block added to the chain, or viewing the balance of cryptocurrency in an account.        
################################################################################
# Streamlit Code
...
##########################################
#Write the wage variables to the Streamlit sidebar by using st.sidebar.write.
...
#write the code that will allow a customer (you, in this case) to send an Ethereum blockchain transaction that pays the hired candidate.
#Call the send_transaction function and pass it three parameters:transaction_hash = send_transaction(w3, account, candidate_address, wage
...
#Inspect the Transaction, using the Streamlit application, type "streamlit run fintech_finder.py"
